...
def main(args):
    ...
    for data in data_lst:
        globals()[f'{data.split("_",1)[0]}'] = pd.read_csv(
            os.path.join(data_path + "/raw/", data)
        )
    ...
    merge_df = pd.merge(
        interaction, albumInfo, on=["album_name", "artist_name"], how="left"
    )
    # 2. 1 + track (on = track_name, how = left)
    merge_df = pd.merge(
        merge_df, trackinfo, on=["track_name", "album_name", "artist_name"], how="left"
    )
    ...
    merge_df.loc[tag_mask, "tag"] = (
        merge_df.loc[tag_mask, "track_tag_list"]
        .map(lambda x: ast.literal_eval(x) if x != None else x)
        .map(lambda x: list(set(x).intersection(set(tag_list))))
    )
    merge_df["tag"] = merge_df["tag"].apply(lambda x: np.nan if x == [] else x)

    # 최종 dataframe
    final_df = merge_df[
        ["user_name", "album_name", "date_uts", "artist_name", "track_name", "tag"]
    ]
    ...
    pickle.dump(
        attributes_dict_list,
        open(data_path + "/artifacts/attributes_dict_list.pkl", "wb"),
    )
    ...
    for i, col in enumerate(attributes[["album_name", "artist_name", "tag"]]):
        ...
        final_df_explode[f"{col}2id"] = final_df_explode[col].map(
            attributes_dict_list[i]
        )

    tag2id_join = final_df["track_name"].map(
        final_df_explode.astype(str)
        .groupby("track_name")["tag2id"]
    ...
    final_df["attributes_id"] = (
        final_df[["album_name2id", "artist_name2id", "tag2id_join"]]
        .astype(str)
    ...
    attributes_json = json.dumps(
        dict(zip(final_df["track2id"], final_df["attributes_id"]))
    )
    ...
    interactions.to_csv(
        data_path + "/artifacts/interaction.txt",
        sep=" ",
        header=False,
