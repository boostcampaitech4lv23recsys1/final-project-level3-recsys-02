...
from utils import (check_path, get_item2attribute_json, get_user_seqs_long,
                   set_seed)
...
def main():
    ...
    user_seq, max_item, long_sequence = get_user_seqs_long(
        args.data_file
    )  # user별 interaction item list, max(itemset), user 구분 없이 전체 interaction item list
    item2attribute, attribute_size = get_item2attribute_json(
        item2attribute_file
    )  # return json, max(attribute_set)
