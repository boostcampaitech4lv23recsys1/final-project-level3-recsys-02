...
class Trainer:
    def __init__(self, model, train_dataloader, eval_dataloader, test_dataloader, args):
        ...
        self.scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer=self.optim, mode="min", factor=0.95, patience=5, verbose=True
        )
        self.patience = 0
    ...
    def predict_sample(self, seq_out, test_neg_sample):
        ...
        # [batch hidden_size]
        test_logits = torch.bmm(test_item_emb, seq_out.unsqueeze(-1)).squeeze(
            -1
        )  # [B 100]
        return test_logits
    ...
    def predict_full(self, seq_out):
        ...
        rating_pred = torch.matmul(
            seq_out, test_item_emb.transpose(0, 1)
        )  # 왜 weight와 matmul?
        return rating_pred
