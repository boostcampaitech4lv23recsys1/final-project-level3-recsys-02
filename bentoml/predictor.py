...
class Predictor:
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        return (
            [recall[0], ndcg[0], recall[1], ndcg[1], recall[3], ndcg[3]],
            str(post_fix),
            pred_list,
        )
    ...
    def get_topk_users(self, target_user, pred_list):
        ...
        target_emb = self.item_embeddings(
            torch.tensor([target_user_items]).squeeze(0).to(self.device)
        )  # seq_len x hidden_size(64)
        ...
        if full_sort:
            ...
            if attribute == None:
                for i, batch in rec_data_iter:
                    ...
                    recommend_output = self.model.finetune(
                        input_ids
                    )  # return hidden states
                    recommend_output = recommend_output[
                        :, -1, :
                    ]  # transformer의 마지막 Sequence값

                    rating_pred = self.predict_full(
                        recommend_output
                    )  # item embedding.weight과 matmul
                ...
                for i, batch in rec_data_iter:
                    ...
                    rating_pred = self.predict_full_attribute(
                        recommend_output, attribute
                    )  # item embedding.weight+attribute embedding.weight과 matmul
