...
class PretrainDataset(Dataset):
    ...
    def split_sequence(self):
        for seq in self.user_seq:  # user별로 split
            input_ids = seq[
                -(self.max_len + 2) : -2
            ]  # keeping same as train set / max_seq_len 만큼 길이 맞추기
            for i in range(len(input_ids)):
                self.part_sequence.append(
                    input_ids[: i + 1]
                )  # sequence를 하나씩 추가하면서 append: (0, [0,1], [0,1,2], ...)
    ...
    def __getitem__(self, index):  # [TODO] Code Review
        ...
        for item in sequence[:-1]:  # part_sequence에서 마지막 sequence제외하고 loop
            ...
            if prob < self.args.mask_p:  # mask_p(0.2)보다 확률이 작으면
                masked_item_sequence.append(
                    self.args.mask_id
                )  # args.mask_id(= max_item + 1) append
                neg_items.append(
                    neg_sample(item_set, self.args.item_size)
                )  # itemset에 없는 item을 neg sample로 1개 append
